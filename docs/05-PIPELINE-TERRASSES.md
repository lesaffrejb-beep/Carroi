...
3. **Ensoleillement** : GRASS GIS `r.sun` + `r.horizon` (scriptable, éprouvé) sur le MNS 0,5 m → raster d'heures de soleil direct par pixel aux trois dates. Alternative validée : plugin UMEP/SEBE (conçu pour l'irradiance au sol). Choisir `r.sun` par défaut (automatisation batch plus simple, pas de dépendance QGIS).
